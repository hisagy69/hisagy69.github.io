const generateId = () => `adam${Math.round(Math.random() * 1e8).toString(16)}`
...
const renderOperation = (operation) => {
  ...
  listItem.innerHTML = `${operation.description}
    <span class="history__money">${operation.amount} Р</span>
...
const updateBalance = () => {
  ...
  const resultExpenses = dbOperation
        ...
        totalMoneyIncome.textContent = resultIncome + ' Р';
        totalMoneyExpenses.textContent = resultExpenses + ' Р';
        totalBalance.textContent = (resultIncome + resultExpenses) + ' Р';
};
